...
interface SmartAccessibilityInputProps {
  onAdd: (nome: string, isNew: boolean) => void;
  placeholder?: string;
  disabled?: boolean;
}

export default function SmartAccessibilityInput({
  onAdd,
  placeholder = "Digite para buscar ou criar nova acessibilidade...",
  disabled = false
}: SmartAccessibilityInputProps) {
  ...
  const handleSelectExisting = (nome: string) => {
    onAdd(nome, false);
    setSearch('');
    setIsOpen(false);
  };
  ...
  const handleCreateNew = async () => {
    if (!isAdmin) {
      alert('Apenas administradores podem criar novas acessibilidades');
      return;
    }
    ...
    try {
      ...
      if (novaAcessibilidade) {
        onAdd(novaAcessibilidade.nome, true);
      ...
      console.error('Erro ao criar acessibilidade:', err);
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      if (filteredOptions.length > 0) {
        ...
        handleSelectExisting(filteredOptions[0].nome);
        ...
        <div className="absolute top-full left-0 w-full bg-white border border-gray-300 shadow-lg max-h-48 overflow-y-auto rounded-lg z-10 mt-1">
          ...
          {filteredOptions.length > 0 && (
            <>
              ...
              {filteredOptions.slice(0, 5).map((acessibilidade) => (
                <div
                  key={acessibilidade.id}
                  onClick={() => handleSelectExisting(acessibilidade.nome)}
          ...
          {shouldShowCreate && (
            <div 
              onClick={isAdmin && !isCreating ? handleCreateNew : () => {
                if (!isAdmin) alert('Apenas administradores podem criar novas acessibilidades');
              }}
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <div className={`text-xs ${
                    ...
                    {isCreating 
                      ? 'Aguarde, salvando a nova acessibilidade...'
                      : isAdmin 
                        ? 'Esta acessibilidade não existe ainda. Clique para criar.'
                        : 'Apenas administradores podem criar novas acessibilidades.'
                    }
                  </div>
                </div>
              </div>
