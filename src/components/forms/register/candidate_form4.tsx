    ...
    return (
        <form id={formId} className="flex-col text-start space-y-8" onSubmit={(e) => {
            ...
            <div className="flex flex-row gap-24">
                <GenericFormField id="candidate_type_register" type="select" options={['W', 'I', 'P']} placeholder="Selecione" required onChange={(e) => setForm4((prev) => ({...prev, necessityType: e.target.value}))} value={form4.necessityType || ""}>Tipo de Necessidade</GenericFormField>
                <GenericFormField id="candidate_sub_type_register" type="select" options={['W', 'I', 'P']} placeholder="Selecione" required onChange={(e) => setForm4((prev) => ({...prev, necessitySubtype: e.target.value}))} value={form4.necessitySubtype || ""}>Subtipo de Necessidade</GenericFormField>
            </div>
