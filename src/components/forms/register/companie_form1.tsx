...
export default function CompanieForm1({ formFunc, formId, initialData }: { formFunc: (data: CandidateForm1Data) => void, formId: string, initialData?: CandidateForm1Data }) {
    return (
        <form className="flex-col text-start space-y-6">
            <h2 className="font-semibold text-[1.3rem]">Informações da Empresa</h2>
            <GenericFormField id="companie_name1_register" placeholder="Digite aqui a razão social" required>Razão social</GenericFormField>
            <GenericFormField id="companie_name2_register" placeholder="Digite aqui o nome fantasia">Nome fantasia</GenericFormField>
            <div className="flex gap-32">
                <GenericFormField id="companie_cnpj_register" placeholder="Digite aqui o CNPJ" required>CNPJ</GenericFormField>
